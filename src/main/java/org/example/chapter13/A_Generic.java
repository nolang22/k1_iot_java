...
/*  === 제네릭(Generic, 일반적인) ===
    ...
    : 클래스나 메서드를 선언할때, 구체적인 타입을 명시하지 않고
        , 사용 시점에 지정할수 있도록 하는 기능
...
class Pair<K, V> {
    ...
    // [접근제어자] <제네릭타입> 반환타입 메서드명(매개변수...) { 구현부 }
    // cf) static 멤버에서는 클래스의 제네릭 타입 매개 변수 사용이 불가
    //      >> static 메서드는 제니릭 타입을 직접 선언 (<T>: 메서드 선언 앞에 필수 명시)
...
public class A_Generic {
    public static void main(String[] args) {
        ...
        Pair<String, Integer> pair3 = new Pair<String, Integer>("Hi", 999);

        // Pair<int, char> pair3 = new Pair<int, char>();
